...
L'analyse de la stabilité des lois de commande pour les robots manipulateurs repose typiquement sur la \textbf{Méthode Directe de Lyapunov}, voir détails au chapitre \ref{chap:stability}. L'objectif est généralement de prouver que l'erreur de suivi de trajectoire converge vers zéro, c'est-à-dire de démontrer la \textbf{stabilité asymptotique globale} de l'origine de la dynamique de l'erreur. Pour y parvenir, on construit souvent une fonction candidate de Lyapunov $V(\mathbf{x})$ basée sur l'énergie totale du système (énergie cinétique + une énergie potentielle artificielle créée par les gains de commande). L'application de cette méthode est grandement facilitée par la structure  de l'équation dynamique des manipulateurs. 
...
\end{align}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ce qui permet généralement de prouver une convergence asymptotique vers $\mathbf{q}_e=0$
