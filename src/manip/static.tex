...
Lorsque les robots sont contrôler en position par des servo-moteurs à chaque joint, chacun de ces moteurs et boucle d'asservissement en position peut être représenté comme une rigidité dans l'espace des joints, i.e. une variation du couple produit par le joint pour un déplacement de ce joint par rapport à la configuration que le contrôleur tente de maintenir:
%%%%%%%%%%%%%%%%
\begin{align}
\delta \tau_i = k_i \delta q_i \quad \Rightarrow \quad \delta \col{\tau} = K_q \, \delta \col{q}
\end{align}
%%%%%%%%%%%%%%%
où $K_q$ est une matrice de rigidité dans l'espace des joints. La variation de l'espace de joint peut être reliée à un déplacement de l'effecteur, et le couple peut-être ramener en terme d'effet à l'effecteur, pour obtenir une compliance apparente à l'effecteur:
%%%%%%%%%%%%%%%%
\begin{align}
...
\end{align}
%%%%%%%%%%%%%%%
où $C_r$ est une matrice de compliance à l'effecteur. La rigidité apparente à l'effecteur, due à un rigidité dans l'espace des joints, est donc égale à :
%%%%%%%%%%%%%%%%
\begin{align}
\col{f}_e = K_r \delta \col{r} \quad \text{avec } K_r = C_r^{-1} = \left[ J K_q^{-1} J^T \right]^{-1}
\end{align}
%%%%%%%%%%%%%%%
Il est à noter que la matrice $C$ de compliance est singulière lorsque le Jacobien est singulier, donc sur une singularité, un robot manipulateur a une ou des directions avec aucune compliance, i.e. infiniment rigide.
...
Note: Le vecteur colonne par lequel on prend les dérivés partielles de $V$ est ici noté avec une transposition pour respecter la convention (voir section \ref{sec:vectoropindex}) pour obtenir un vecteur-colonne $\col{g}$ $n \times 1$ plutôt que un vecteur-rangé $1 \times n$.

Le nouveau terme provient du fait que la variation de l'energie interne du système n'est pas nulle dans ce cas:
%%%%%%%%%%%%%%%%
\begin{align}
