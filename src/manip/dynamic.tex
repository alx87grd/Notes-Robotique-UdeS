...
Ce chapitre traite de la relation entre la l'évolution temporelle de la position d'un robot et les divers forces appliquées sur celui-ci. Notre outil mathématique principal pour décrire cette relation est un système d'équations différentielles qui relie des variables d'accélération du robot aux autres variables qui influence les forces internes et externes impliqués.

\video{Dynamique des robots manipulateurs}{https://youtu.be/6z3grrVNBj4}

Obtenir cette relation est utile lorsqu'on désire prédire des capacités dynamiques d'un système, simuler numériquement l'évolution temporelle d'un système dans des conditions connues ou bien synthétiser des lois de commandes.
...
La matrice $B(q)$ est une matrice de transformation qui relie le vecteur $u$,  représentants les entrées de notre système, et les forces généralisées $\col{\tau}$ associées dans le systèmes de coordonnées $\col{q}$ utilisé pour décrire les équations:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
...
La Figure \ref{fig:actuatormatrix} illustre qualitativement divers architecture d'actionneurs qui mène à différentes structures pour $B$:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
	\centering
