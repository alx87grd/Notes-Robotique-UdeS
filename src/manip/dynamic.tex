...
%
\begin{align}
H(\col{q}) \col{\ddot{q}} + C(\col{q},\col{\dot{q}}) \col{\dot{q}} + d(\col{q}, \col{\dot{q}}) + \col{g}(\col{q}) = B(\col{q}) \col{\tau}  + J^T(\col{q}) \col{f}_e
\label{eq:manipulator}
\end{align}
%
...
\begin{table}[htbp]
	...
	\caption{Nomenclature pour la dynamique dans l'espace des joints}	% Table caption must be placed on top of the table %
        ...
        \hline \hline
			...
			\hline \hline
			$\col{\tau}$    &  :  & Forces/couples des actionneurs                                   & $m$  \\
			...
			$\col{f}_e$     &  :  & Forces à l'effecteur                       & $o$  \\
...
\begin{align}
c = C(\col{q},\col{\dot{q}}) \col{\dot{q}} + d(\col{q}, \col{\dot{q}}) + \col{g}(\col{q})
\end{align}
%
...
%
\begin{align}
H \col{\ddot{q}} + C \col{\dot{q}} + D \col{\dot{q}} + \col{g} =  \underbrace{ J_a^T(\col{q}) R^T }_{B(\col{q})}  \col{\tau} + J_e^T(\col{q}) \col{f}_e + J_a^T(\col{q}) \col{f}_a
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.90\textwidth]{fig/actuatormatrix.png}
...
%
L'énergie cinématique du système est directement reliée à la matrice d'inertie $H$ et la vitesse des joints $\dot{\col{q}}$:
%
\begin{align}
