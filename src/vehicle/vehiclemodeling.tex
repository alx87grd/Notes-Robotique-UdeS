...
Contrairement au modèle cinématique, le modèle dynamique prend en compte les forces, les masses et les inerties. On utilise le modèle planaire 3 DDL, avec les vitesses exprimées dans le repère du corps : $u$ (longitudinale), $v$ (latérale) et $r$ (vitesse angulaire de lacet). En considérant seulement les forces des pneus et une traînée aérodynamique, les équations sont:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
...
On projette ces vitesses dans le repère local de la roue ($w$). La roue avant ici n'a pas de rotation relative, mais pour la roue avant (braquée de $\delta$), on applique une rotation inverse :
\begin{align}
