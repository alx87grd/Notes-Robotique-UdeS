...
\begin{enumerate}
    ...
    \item \textbf{Conduite sur glace (Rallye) :} Vous prenez un virage serré sur une chaussée glissante. La géométrie ne suffit plus. Il faut comprendre la \textbf{physique} : l'inertie, la friction, le transfert de masse. Si vous tournez le volant, la voiture ne va pas nécessairement suivre la direction des roues.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Action Cinématique (Abstraite) :} On commande directement des vitesses (ex: "Avance à 1 m/s", "Tourne à 0.5 rad/s"). C'est l'hypothèse simplificatrice utilisée par la plupart des planificateurs de trajectoire.
...
Formellement, l'ensemble des entrées de commande admissibles est noté $\mathcal{U}$. Le vecteur $\mathbf{u} \in \mathcal{U}$ représente les variables d'entrée des équations du modèle utilisé. Lorsque qu'on utilise des actions abstraites, l'hypothèse sous-jacente est qu'il existe des contrôleurs de bas niveau suffisamment performants pour exécuter ces actions quasi-instantanément.
...
Ces modèles utilisent la seconde loi de Newton ($F=ma$). Ils répondent à la question : \textit{"Si j'applique telles forces, comment vont évoluer mes coordonnées?"}. Lorsque les actions sont des forces, ces équations prennent la forme suivante, i.e. des équations différentielles d'ordre deux:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
...
Parfois, on s'intéresse seulement à l'évolution des vitesses d'un véhicule. On peut alors travailler avec des équations qui n'inclut que l'évolution des vitesses, sans prédire l'évolution de la configuration. Dans cette situation, on peut utiliser un vecteur de vitesses $\mathbf{\nu}$ et des équations différentielles d'ordre un:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
    \dot{\mathbf{\nu}} = f_v(\dot{\mathbf{\nu}},\mathbf{u})
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Aucune propriétés ou force de doit dépendre d'une variable configuration pour pouvoir utiliser cette forme. On appelle parfois ces variables les coordonnées ignorables. Un exemple est la position $x$ d'un avion, il est possible de calculer l'évolution des vitesses de l'avion sans garder en mémoire l'évolution de sa position.
...
\begin{itemize}
    ...
    \item \textbf{Pour un modèle dynamique}, l'état doit inclure la configuration et les vitesses (linéaire et angulaire) : $\mathbf{x} = [\mathbf{q}, \dot{\mathbf{q}}]^T$. Sans connaître la vitesse actuelle, on ne peut pas prédire la position future.
\end{itemize}
...
\begin{enumerate}
    \item La matrice d'allocation $\mathbf{B}_b$ est \textbf{constante} (elle ne dépend plus de $\theta$). C'est un avantage majeur pour la synthèse de commande.
    ...
    \item La gravité $\mathbf{g}_b(\mathbf{q})$ est le seul terme qui dépend de l'orientation $\theta$. Elle représente la projection du vecteur poids sur les axes du drone.
\end{enumerate}
...
Le modèle présenté ici est basé sur les travaux de Fossen \cite{fossen2021handbook}, spécifiquement les modèles de manœuvre à basse vitesse.
